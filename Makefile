...
dev.migrate.cms:
	docker-compose exec cms bash -c 'source /edx/app/edxapp/edxapp_env && cd /edx/app/edxapp/edx-platform/ && paver update_db'
...
dev.shell.cms:
	docker-compose exec cms env TERM=$(TERM) /edx/app/edxapp/devstack.sh open

dev.shell.cms_watcher:
	docker-compose exec cms_watcher env TERM=$(TERM) /edx/app/edxapp/devstack.sh open
...
e2e-tests: dev.up.lms+cms+firefox+chrome ## Run the end-to-end tests against the service containers.
...
e2e-tests.with-shell: dev.up.lms+cms+firefox+chrome ## Start the end-to-end tests container with a shell.
